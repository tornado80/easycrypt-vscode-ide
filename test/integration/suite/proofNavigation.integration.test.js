...
const { getAllStatements } = require('../../../out/statementParser');
...
async function createKemDemFixtureCopy(mutateSource) {
  const srcDir = path.resolve(__dirname, '..', '..', 'kem-dem', 'left-or-right');
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'easycrypt-proofnav-kemdem-e2e-'));
...
function locateInjectedFailure(source) {
  const statements = getAllStatements(source, 0, source.length);
  const failingStatementIndex = statements.findIndex((statement) =>
    statement.text.includes('undefined_symbol')
  );

  assert.ok(failingStatementIndex >= 0, 'Expected to find injected undefined_symbol statement');

  const expectedOffsetBeforeFailure =
    failingStatementIndex > 0 ? statements[failingStatementIndex - 1].endOffset : 0;

  const failingStatement = statements[failingStatementIndex];
  const failingStatementStartLine = source.slice(0, failingStatement.startOffset).split('\n').length - 1;

  return {
    statements,
    failingStatementIndex,
    failingStatement,
    expectedOffsetBeforeFailure,
    failingStatementStartLine,
  };
}

function countMockFailTriggers(statementText) {
  let count = 0;
...
function computeMockFailCounterForStatementIndex(statements, targetStatementIndex) {
  assert.ok(targetStatementIndex >= 0 && targetStatementIndex < statements.length, 'Invalid target statement index');

  let counter = 0;
  for (let index = 0; index <= targetStatementIndex; index += 1) {
    counter += countMockFailTriggers(statements[index].text);
  }

  assert.ok(counter > 0, 'Expected a positive fail counter for target statement');
  return counter;
}
...
describe('Interactive Proof Navigation Integration (mock easycrypt)', function () {
  ...
  describe('Stop On First Error (serial replay)', function () {
    it('goToCursor forward failure stops at first failing statement with no retry', async function () {
      process.env.MOCK_EC_FAILING_GOAL_MARKER = 'STOP_ON_FIRST_ERROR_GOAL';
      process.env.MOCK_EC_FAILING_ERROR_MARKER = 'STOP_ON_FIRST_ERROR_ERR';

      const mutateSource = (source) => {
        const needle = '  by sim.';
        const index = source.indexOf(needle);
        assert.ok(index >= 0, 'Expected a stable by sim. line in KEMDEM_lor.ec');
        return source.slice(0, index) + '  undefined_symbol.' + source.slice(index + needle.length);
      };

      const { filePath, source, cleanup } = await createKemDemFixtureCopy(mutateSource);
      const failureInfo = locateInjectedFailure(source);
      ...
      try {
        ...
        const executionOffset = await vscode.commands.executeCommand('easycrypt._getExecutionOffset');
        assert.equal(
          executionOffset,
          failureInfo.expectedOffsetBeforeFailure,
          `Expected execution offset to stop before failing statement (${failureInfo.expectedOffsetBeforeFailure}), got ${executionOffset}`
        );

        const verifiedRange = await vscode.commands.executeCommand('easycrypt._getVerifiedRange');
        assert.ok(verifiedRange, 'Expected verified range after partial forward replay');
        assert.ok(
          verifiedRange.end.line <= failureInfo.failingStatementStartLine,
          `Expected verified range to stop before failing line ${failureInfo.failingStatementStartLine}, got ${JSON.stringify(verifiedRange)}`
        );

        const snapshot = await vscode.commands.executeCommand('easycrypt._getProofStateSnapshot');
        const outputText = Array.isArray(snapshot?.outputLines) ? snapshot.outputLines.join('\n') : '';
        assert.ok(
          outputText.includes('STOP_ON_FIRST_ERROR_ERR') && outputText.includes('STOP_ON_FIRST_ERROR_GOAL'),
          `Expected proof snapshot to include failure + goal markers, got: ${outputText.slice(-1200)}`
        );

        const sendsAfter = await vscode.commands.executeCommand('easycrypt._getSendCommandCount');
        const startsAfter = await vscode.commands.executeCommand('easycrypt._getProcessStartCount');

        const expectedSends = failureInfo.failingStatementIndex + 1;
        assert.equal(
          sendsAfter - sendsBefore,
          expectedSends,
          `Expected exactly ${expectedSends} sends (up to and including failing statement), got delta=${sendsAfter - sendsBefore}`
        );
        assert.equal(
          startsAfter - startsBefore,
          0,
          `Expected no restart during forward failure path, got delta=${startsAfter - startsBefore}`
        );
      } finally {
        delete process.env.MOCK_EC_FAILING_GOAL_MARKER;
        delete process.env.MOCK_EC_FAILING_ERROR_MARKER;
        await vscode.commands.executeCommand('easycrypt.stopProcess');
        await cleanup();
      }
    });

    it('recovery replay stops at first failing statement with single restart and no fallback', async function () {
      process.env.MOCK_EC_FAILING_GOAL_MARKER = 'RECOVERY_STOP_GOAL';
      process.env.MOCK_EC_FAILING_ERROR_MARKER = 'RECOVERY_STOP_ERR';

      const { filePath, source: cleanSource, cleanup } = await createKemDemFixtureCopy();

      try {
        await vscode.commands.executeCommand('easycrypt.stopProcess');

        const statements = getAllStatements(cleanSource, 0, cleanSource.length);
        const desiredFailingStatementOrdinal = Math.min(Math.max(2, 80), statements.length - 1);
        const desiredFailingStatementIndex = desiredFailingStatementOrdinal - 1;
        const failAtCommandIndex = computeMockFailCounterForStatementIndex(statements, desiredFailingStatementIndex);

        const failingStatement = statements[desiredFailingStatementIndex];
        const expectedOffsetBeforeFailure = statements[desiredFailingStatementIndex - 1].endOffset;
        const failingStatementStartLine = cleanSource.slice(0, failingStatement.startOffset).split('\n').length - 1;

        const cleanDoc = await vscode.workspace.openTextDocument(vscode.Uri.file(filePath));
        await vscode.languages.setTextDocumentLanguage(cleanDoc, 'easycrypt');
        const editor = await vscode.window.showTextDocument(cleanDoc);

        await vscode.commands.executeCommand('easycrypt.resetProof');

        const initialTargetPos = cleanDoc.positionAt(cleanSource.length);
        editor.selection = new vscode.Selection(initialTargetPos, initialTargetPos);
        const forward = await vscode.commands.executeCommand('easycrypt.goToCursor');
        assert.ok(forward && forward.success, `Expected clean forward goToCursor success, got: ${JSON.stringify(forward)}`);

        const prefixOffset = await vscode.commands.executeCommand('easycrypt._getExecutionOffset');
        assert.ok(
          prefixOffset > failingStatement.endOffset,
          `Expected verified prefix past replay failure point, got offset=${prefixOffset}`
        );

        process.env.MOCK_EC_FAIL_AT_COMMAND_INDEX = String(failAtCommandIndex);

        const offsetBeforeRecovery = await vscode.commands.executeCommand('easycrypt._getExecutionOffset');
        assert.equal(
          offsetBeforeRecovery,
          prefixOffset,
          `Expected execution offset to persist before recovery. expected=${prefixOffset}, actual=${offsetBeforeRecovery}`
        );
        ...
        assert.ok(recovery && recovery.success === false, `Expected forceRecovery failure at injected statement, got: ${JSON.stringify(recovery)}`);

        const offsetAfterRecovery = await vscode.commands.executeCommand('easycrypt._getExecutionOffset');
        assert.equal(
          offsetAfterRecovery,
          expectedOffsetBeforeFailure,
          `Expected recovery to stop before failing statement (${expectedOffsetBeforeFailure}), got ${offsetAfterRecovery}`
        );

        const verifiedRange = await vscode.commands.executeCommand('easycrypt._getVerifiedRange');
        assert.ok(verifiedRange, 'Expected verified range after recovery failure');
        assert.ok(
          verifiedRange.end.line <= failingStatementStartLine,
          `Expected verified range to stop before failing replay line ${failingStatementStartLine}, got ${JSON.stringify(verifiedRange)}`
        );

        const snapshot = await vscode.commands.executeCommand('easycrypt._getProofStateSnapshot');
        const outputText = Array.isArray(snapshot?.outputLines) ? snapshot.outputLines.join('\n') : '';
        assert.ok(
          outputText.includes('RECOVERY_STOP_ERR') && outputText.includes('RECOVERY_STOP_GOAL'),
          `Expected recovery snapshot to include failing replay markers, got: ${outputText.slice(-1200)}`
        );
        ...
        assert.equal(
          startsAfter - startsBefore,
          1,
          `Expected exactly one restart at recovery start, got delta=${startsAfter - startsBefore}`
        );

        const expectedSends = desiredFailingStatementOrdinal;
        assert.equal(
          sendsAfter - sendsBefore,
          expectedSends,
          `Expected recovery to send exactly ${expectedSends} statements with no fallback tail, got delta=${sendsAfter - sendsBefore}`
        );
      } finally {
        delete process.env.MOCK_EC_FAILING_GOAL_MARKER;
        delete process.env.MOCK_EC_FAILING_ERROR_MARKER;
        delete process.env.MOCK_EC_FAIL_AT_COMMAND_INDEX;
        await vscode.commands.executeCommand('easycrypt.stopProcess');
        await cleanup();
      }
    });
