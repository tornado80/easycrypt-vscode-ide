...
const MODULE_NOT_FOUND_PATTERN = /module not found/i;
...
async function configureMockEasyCrypt() {
  ...
  await cfg.update('diagnostics.delay', 80, vscode.ConfigurationTarget.Global);
}
...
describe('Module Imports E2E (mock easycrypt strict import mode)', function () {
  this.timeout(120_000);

  before(async function () {
    process.env.MOCK_EC_STRICT_IMPORTS = '1';
  ...
  after(async function () {
    delete process.env.MOCK_EC_UNDO_FAIL;
    delete process.env.MOCK_EC_STRICT_IMPORTS;
    await vscode.commands.executeCommand('easycrypt.stopProcess');
  });
  ...
  it('reports actionable diagnostics for missing imports', async function () {
    ...
    try {
      ...
      assert.ok(
        diagnostics.some((diag) => /include-path mismatch|directory\/include-path mismatch/i.test(diag.message)),
        `Expected actionable include-path hint, got: ${JSON.stringify(diagnostics.map((d) => d.message))}`
      );
    } finally {
  ...
  it('live unsaved checks keep import resolution stable', async function () {
    const kemDemPath = path.resolve(
      __dirname,
      '..',
      '..',
    ...
    const { doc, editor } = await openEasyCryptDocument(kemDemPath);
    ...
    assert.ok(Array.isArray(diagnostics) && diagnostics.length > 0, 'Expected diagnostics from unsaved live check');
    assert.equal(
      diagnostics.some((diag) => MODULE_NOT_FOUND_PATTERN.test(diag.message)),
      false,
      `Expected no import-resolution diagnostics during unsaved check, got: ${JSON.stringify(diagnostics.map((d) => d.message))}`
    );
  });

  it('preemption publishes latest live-check result only', async function () {
    const project = await createTempProject({
      'preemption.ec': [
        'lemma t : true.',
        'proof.',
    ...
    try {
      ...
      await editor.edit((editBuilder) => {
        ...
        editBuilder.replace(range, 'undefined_symbol.');
      });
      ...
      const diagnostics = await waitFor(async () => {
        ...
        const hasParseError = errors.some((diag) => /parse error/i.test(diag.message));
        const hasSyntaxError = errors.some((diag) => /syntax error/i.test(diag.message));
        return hasParseError && !hasSyntaxError ? errors : undefined;
      }, 25_000, 120);

      assert.ok(Array.isArray(diagnostics) && diagnostics.length > 0, 'Expected final diagnostics from latest edit');
      assert.ok(
        diagnostics.every((diag) => /parse error/i.test(diag.message)),
        `Expected latest parse-error diagnostics, got: ${JSON.stringify(diagnostics.map((d) => d.message))}`
      );
    } finally {
      await project.cleanup();
    }
  ...
  it('stepBackward fallback recovery keeps imports resolvable', async function () {
    ...
    // Restart process so MOCK_EC_UNDO_FAIL is observed by the mock REPL.
    await vscode.commands.executeCommand('easycrypt.stopProcess');
    ...
    assert.ok(backward && backward.success, `Expected stepBackward success via recovery fallback, got: ${JSON.stringify(backward)}`);

    assert.equal(
      hasModuleNotFoundDiagnostics(doc.uri),
      false,
      `Expected no module-not-found diagnostics after fallback recovery, got: ${JSON.stringify(getErrorDiagnostics(doc.uri).map((d) => d.message))}`
    );
  ...
  it('switching between directories rebinds session context for imports', async function () {
    const project = await createTempProject({
      'dir-a/Helper_A.eca': '(* helper module A *)\n',
      'dir-a/Main_A.ec': [
      ...
      'dir-b/Helper_B.eca': '(* helper module B *)\n',
      'dir-b/Main_B.ec': [
